...
describe('toCSVStream', () => {
  ...
  describe('Alternative configuration', () => {
    let altInput;
    beforeEach(() => {
      altInput = _(fs.createReadStream(
        './fixtures/Enpass_2016-01-11_01-42-13.txt', 'utf8'));
    });

    it('should be able to use alternative configuration for URL cleaning', (done) => {
      const altOutput = toCSVStream({
        cleanUrls: false,
      })(altInput);
    ...
    it('should be able to use alternative configuration for empty entries', (done) => {
      const altOutput = toCSVStream({
        removeUnuseful: false,
      })(altInput);
    ...
    it('should be able to use alternative configuration for name cleaning', (done) => {
      const altOutput = toCSVStream({
        cleanNames: false,
      })(altInput);

      altOutput.split()
      .filter(x => x.includes('Generated password for website1.com'))
      .pull((err, line) => {
        assert.ok(line.length > 0);
        done();
      });
  ...
  it('should not remove entries that fall back to default email');
});
